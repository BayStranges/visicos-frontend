<template>
  <div class="layout" @touchstart="onLayoutTouchStart" @touchend="onLayoutTouchEnd">
    ...
    <div v-if="mobileFriendsOpen" class="mobile-overlay" @click="closeFriendsPanel"></div>

    <section class="friends" :class="{ 'is-open': mobileFriendsOpen }">
      <div class="friends-mobile-head">
        <div class="friends-mobile-title">Arkadaslar</div>
        <button class="friends-mobile-close" @click="closeFriendsPanel">Kapat</button>
      </div>
        ...
        <div class="panel-header">
          <button class="panel-btn" @click="showNotifications = !showNotifications" aria-label="Bildirimler">
            <svg class="panel-icon" viewBox="0 0 24 24" aria-hidden="true">
              <path d="M12 22a2.5 2.5 0 0 0 2.45-2h-4.9A2.5 2.5 0 0 0 12 22Zm6.36-6a2 2 0 0 0 1.64-1.96c0-.97-.66-1.77-1.56-1.96V10a6.44 6.44 0 0 0-5.02-6.3V3a1.5 1.5 0 0 0-3 0v.7A6.44 6.44 0 0 0 5.4 10v2.08A2 2 0 0 0 3.8 14c0 1.02.78 1.86 1.78 2l1.12.16h11.66Z" />
            </svg>
          </button>
        </div>
    ...
    <section class="activity">
      <div class="panel">
        <div class="panel-title">Simdi Aktif</div>
...
const toggleFriendsPanel = () => {
  mobileFriendsOpen.value = !mobileFriendsOpen.value;
};

const closeFriendsPanel = () => {
  mobileFriendsOpen.value = false;
};

const onLayoutTouchStart = (event) => {
  if (!event.touches || event.touches.length !== 1) return;
  const touch = event.touches[0];
  layoutTouchStart.value = { x: touch.clientX, y: touch.clientY };
};

const onLayoutTouchEnd = (event) => {
  const touch = event.changedTouches?.[0];
  if (!touch) return;
  const dx = touch.clientX - layoutTouchStart.value.x;
  const dy = touch.clientY - layoutTouchStart.value.y;
  if (Math.abs(dy) > 80) return;
  if (layoutTouchStart.value.x < 24 && dx > 70) {
    mobileFriendsOpen.value = true;
  } else if (dx < -70 && mobileFriendsOpen.value) {
    mobileFriendsOpen.value = false;
  }
};
